...
import { loadSessionRecording, playSessionRecording } from '@agenttalk/observability/recordings/playback';
...
describe('recording playback', () => {
  ...
  it('should persist runtime events and replay the resulting final state', async () => {
    ...
    expect(raw.split('\n').filter(Boolean)).toHaveLength(7 + 1);
    ...
    expect(state.processedEvents).toBe(7);
