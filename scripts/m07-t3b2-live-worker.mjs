...
// One real cli-exec worker is driven a `team_work_assign`; the orchestrator
...
async function run() {
  ...
  await registry.createAgent('cli-worker-1', { provider: 'cli-exec', providerName: 'gemini' });
  await registry.activateAgent('cli-worker-1');

  const llmAgentPath = path.join(repoRoot, '../agentalk-mcp-client/llm-agent.mjs');
  const harness = spawn('node', [llmAgentPath, '--agentId', 'cli-worker-1', '--provider', 'gemini', '--execution-mode', 'persistent'],
  ...
  registry.on('exec_result', (r) => {
    if (r.agentId === 'cli-worker-1') { execText = r.text; log(`[Test] exec_result (raw agy reply): ${JSON.stringify(r.text)?.slice(0, 800)}`); }
  });
  registry.on('mcp_tool_call', (call) => {
    if (call.agentId === 'cli-worker-1' && (call.name === 'submit_work_result' || call.name === 'submit_work_response')) {
      log(`[Test] ${call.name}: ${JSON.stringify(call.args)?.slice(0, 600)}`);
    }
  });

  log(`[Test] Sending team_work_assign to cli-worker-1 (taskId=${taskId}) ...`);
  await registry.sendProtocol('cli-worker-1', 'EVT', {
