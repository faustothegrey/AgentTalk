...
async function run() {
  ...
  console.log(`Starting live MCP exec-RPC smoke test for provider: ${providerName}...`);
  ...
  const harness = spawn('node', [llmAgentPath, '--agentId', 'mcp-planner-1', '--provider', providerName, '--execution-mode', 'persistent'],
  ...
  if (success) {
    console.log(`TEST PASSED: Live MCP exec-RPC turn successfully completed for ${providerName}.`);
    process.exit(0);
  } else {
