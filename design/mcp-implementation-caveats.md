...
**Date:** 2026-06-18
**Related:** `design/mcp-orchestration-proposal.md`, `design/mcp-cli-capability-assessment.md`
...
- **stdio MCP works on all three CLIs** (Claude 2.1.181, Codex 0.133.0, Gemini 0.41.2).
...
- **The gating spike (§10F) is self-contained and carries no production risk** — a mock
  WS server, a basic `mcp-bridge.js`, and spawning Codex/Claude against it. Nothing in
...
error?)? Is the CLI process killed or left idle until the next prompt?
...
falling back to HTTP/SSE" (implying **CLIs connect directly** over WS) *and*
"Stdio-Everywhere Adapter Bridge" (implying **CLIs speak stdio** to a local bridge that
speaks WS to the server). These are different architectures; only the second is sound.

- **Decision needed:** do CLIs connect to the central server directly (over WS/HTTP), or
  does every CLI speak stdio to a local `mcp-bridge.js` that relays to the central
  server?
- **Recommendation:** stdio→bridge→(WS/HTTP) for **all** providers, not just Codex. Then
  no CLI's network-MCP-client support is ever on the critical path, and the bridge↔server
...
this irrelevant because each CLI only ever needs **stdio**, which is verified working.
...
trust doesn't also enable behaviors we don't want in spawned agents.
...
`mcp-cli-capability-assessment.md` still frames "can each CLI be an HTTP/SSE client?" as a
