...
- The MCP-attach path (`agentalk-mcp-client` / `llm-agent.mjs`) is the current friction: an attach ritual per agent,
  ...
  `exec_rpc`; LB-92, BL-045 deferred). This is slowing AgentTalk development.
...
- **`POST /api/agents` must accept `providerName`** — today it reads only `{id, provider, model}` (`server.ts:593`) and
  ...
  creating OpenRouter agents through the product. **Promoted to → BL-046** (was BL-044 finding #2).
- (Arbiter reachability and the Google tool-schema 400 stay in BL-044; neither blocks the OpenRouter direction.)
...
full CLI dev sessions, PO-batoned) are unaffected. Source: this session's TL-005 findings, LB-91/LB-92, BL-044/BL-045,
