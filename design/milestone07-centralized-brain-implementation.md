...
**Status:** **T3b-1 REVIEWED — code GREEN but a premise issue surfaced (FIND-T3b1-1) → needs Fausto's call before merge.** Rows T3b-1.1–1.5 + 1.7 **VERIFIED** (159/159, tsc clean, API byte-for-byte, recovery + no-resend tests pass); 1.6 live **HELD**. **FIND-T3b1-1:** the change correctly flattens the `!currentConversation` path but the **consensus/planning path was already bounded** (flat 2881 B; `buildLatestTurnPrompt`≡`buildPrompt` there) → **LB-4 overstated**; T3b-1's win is direct-chat + the recovery/`maintainsSession` plumbing for T3b-2, not consensus. Architect premise error, not implementer fault. Decision pending. T3a MERGED; spikes T3-S1/S2 DONE; D5 set. Findings: native `agy --continue` session **works** through exec-RPC (option 2 viable), **but** the driver currently **resends full history** (option 1) and native session **doesn't survive a harness restart** (ephemeral home). **D2 recommendation = hybrid: native session steady-state + resend-based recovery fallback** (see D2 block; T3b owns it). Reconnect+exec-RPC delivery gap surfaced → IMP-T3b-1. T3a MERGED (orchestrator `e9186e1`, harness `17edffc`; master 157/157). T1+T2 DONE/merged (T2.4 deferred). M06 closed; R1 GREEN.
...
error, not an implementer fault.** Decision (Fausto) pending: keep+reframe (recommended) vs re-scope; LB-4 correction held for that call.
