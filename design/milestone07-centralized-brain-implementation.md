...
**Status:** **Task M07-T3a — ROUND 1 REVIEWED → back to implementer (NOT merged).** Verdicts: **T3a.1 ✅ · T3a.3 ✅ · T3a.5 ✅** (reviewer ran live agy) — **T3a.2 ❌ · T3a.4 ❌ · T3a.6 ❌**. The exec-RPC inversion **works live**, but the required mocked unit test (`cli-exec-agent.test.ts`) is **broken** (asserts a push; impl is pull) → suite **156/1 fail**, and **nothing is committed** (both repos). `tsc -b` clean. **3 fixes for round 2** (see findings block). T1 + T2 **DONE/merged** (T2.4 deferred → backlog, Google `UNAVAILABLE` 2026-06-21). M06 closed; R1 GREEN.
...
| **T3a.2** **`CliExecCompleter`** (orchestrator): exec-RPC over the existing WS/MCP transport → awaits `{text,usage}`; stateless single-shot (R3 scaffolding). **Mocked-transport unit test.** | **done** | **REFUTED ❌** | The **code works** (proven live, T3a.5) — but the *required* mocked-transport unit test is **broken and failing**. `cli-exec-agent.test.ts:49` asserts `sendProtocol(...,'EVT',{type:'exec_rpc'})` (a **push**), while `CliExecCompleter` is **pull-based** (`agent.queueExecTurn` → delivered on `awaitExecTurn`); `sendProtocol` is never called → `Number of calls: 0`. The test's own comment ("without enqueuing it back to the agent") contradicts the impl. The DoD deliverable (passing mocked-transport test) is not met. **Fix:** rewrite the test to drive the pull path (queue → `awaitExecTurn`/`await_turn` → `submit_exec_result` → assert the translated tool call) and make it pass. |
...
| **T3a.4** **Flag/config (coexistence, D1):** `provider:'cli-exec', providerName:'gemini'` takes the exec-RPC path; default behaviour unchanged / off by default. **Mocked CI test** drives a planner turn end-to-end via the driver+CliExecCompleter. | **done** | **REFUTED ❌ (behaviour OK, test deliverable fails)** | The **coexistence routing works**: `registry.ts` starts the driver for `provider:'cli-exec'` with a `CliExecCompleter`; `await_turn` pulls from `awaitExecTurn` only for cli-exec; default/other providers unchanged + off by default. Proven live (T3a.5). **But** the DoD requires a "Mocked CI test [that] drives a planner turn end-to-end via the driver+CliExecCompleter" — that test is the **failing** `cli-exec-agent.test.ts` (see T3a.2). So the test deliverable is unmet → REFUTED until that test passes. **Also fix:** remove the leftover debug `console.log("[Registry] await_turn called … pendingTurns=…")` in the production hot path. |
...
| **T3a.6** **No regression:** full suite green; M05/M06 attach + API (T1/T2) paths unchanged; `tsc -b` clean **committed**. | **done** | **REFUTED ❌** | Claim "npm test … perfectly green" + "committed" are **both false**. (1) Ran `npm test` myself → **1 failed / 156 passed (157)** — the failure is the implementer's own `cli-exec-agent.test.ts` (see T3a.2). The suite is **not** green. (2) **Nothing is committed** — `git log master..HEAD` is empty; all changes (orchestrator repo **and** `agentalk-mcp-client`) sit in the working tree only. DoD requires green **committed**. `tsc -b` **is** clean (exit 0), so that part holds. **Fix:** repair the test → 157/157 green, then commit claim-only on `m07-t3a-cli-exec` in **both** repos. |
