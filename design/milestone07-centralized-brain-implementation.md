...
**Status:** **T3c — ALL ROWS T3c.1–T3c.5 VERIFIED ✅ (reviewer, by running); NOT yet merged (awaiting merge of `m07-t3c-contract-bump` in both repos).** Contract bumped v3→4 (added `submit_exec_result`; hash `97abf6…` independently recomputed = match; byte-identical both repos); drift guard has teeth (fails on master, passes on v4); v4 handshake verified live + mismatch rejected; orchestrator `tsc -b` exit 0 + vitest 162/162, harness eslint clean + vitest 4/4. Gemini implemented (claim-only, ledger discipline correct). **➡️ NEXT: merge both repos → master, then M07-T4 (retire client-side semantic logic).**
<br>**Prior:** **T3b-2 — ALL ROWS 2.1–2.6 VERIFIED ✅ (incl. 2.5 LIVE); MERGED** (AgentTalk `534b4ef` / harness `c15d7c7`). Round 2 fixed B1–B5 (`TeamCoordinator` reverted; worktree driver-side, gated to cli-exec; harness worker handler restored; `tsc -b` clean; tests hermetic; `\\n` joins restored). 2.5 live agy worker turn ran 2026-06-21: exec-RPC → real agy → wrote `WORKER_PROOF.txt` with exact expected bytes in a per-task worktree, recorded (`m07-t3b2-live-worker.log`), repo clean after. No-regression re-confirmed: `tsc -b` exit 0 + vitest 160/160. **➡️ NEXT: merge both repos (AgentTalk + agentalk-mcp-client) to master, then T3c (wire-contract bump).** Surfaced FIND-T3b2-1 (agy nests its own worktree — worker-prompt cleanup → backlog, non-blocking). T3b-1 DONE/merged `ff9296d`; T3a + spikes T3-S1/S2 DONE; D5 set; T1+T2 DONE/merged. M06 closed.
...
| **DEV-4** | note | T4b-2 | Found and fixed a bug where `registry.ts`'s `await_turn` handling still checked for `agent.provider === 'cli-exec'` exclusively. Updated it to `['cli-exec', 'gemini', 'claude', 'codex'].includes(agent.provider)`. | |
...
|---|---|---|---|
| **T4a.1 — Live cli-exec consensus gate.** New script (cli-exec analog of `test-live-gate`): 2 cli-exec **agy** planners + 1 cli-exec worker, server-side brain, **full flow** fact_collection → discussion → proposal → acceptance → submit_plan → confirm → worker exec → `team_task` **completed**, all via exec-RPC. **Recorded** (log). | done | **VERIFIED (functional) ⚠️ — FIND-T4a-1 blocks merge** | Ran `scripts/test-live-cli-exec-team.mjs` **live, exit 0** (`m07-t4a-live-consensus.log`). Tool-call order proves a **genuine full protocol** (both planners `ack_planning_protocol`→`fact_collection_end`→`send_to_agent`→`agreement_proposal`(a)→`agreement_acceptance`(b)→`submit_plan`→worker `submit_work_response`/`submit_work_result` → **completed**); all agents `provider:'cli-exec'` (driver path, not attach). Minor: planner-b sent a redundant late `submit_plan` (harmless). **FIND-T4a-1 (cleanup) — round 1 path-scoped fix REFUTED (leaked a sibling agy worktree); round 2 snapshot-diff fix NOW VERIFIED ✅** — reviewer re-ran the gate **twice** (agy nondeterministic); both exit 0 + **zero pollution** after each (worktrees/branches/`/tmp` all clean). No-regression re-confirmed: `tsc -b` 0, vitest **163/163**. ⇒ **all T4a rows VERIFIED — ready to merge** (AgentTalk only; harness untouched). |
...
|---|---|---|---|
| **T4b-2.1 — Route production providers to the driver.** UI-created `gemini`/`claude`/`codex` agents get the cli-exec `InProcessAgentDriver` (exec-RPC), not the wire/semantic path — at `registry.activateAgent` and/or the server create/start path. Define the mapping; keep `api` untouched. | done | **not-started** | A `gemini`/`claude`/`codex` agent created via the server API starts an `InProcessAgentDriver` (log) and completes a turn via `exec_rpc`, not semantic EVTs. |
| **T4b-2.2 — UI still works (behavior preserved).** Prove a real agent turn **and** a team/consensus turn through the migrated path (the web UI / server API end-to-end). Recorded. | proved via scripts using provider directly | **not-started** | Live/recorded: single-agent turn + a 2-planner+worker team reaches `completed` through the migrated production path. |
| **T4b-2.3 — No regression.** Full suite + `tsc -b` green, committed; existing api/cli-exec/attach tests still pass (attach not yet deleted). | done | **not-started** | `tsc -b` 0; vitest all-pass; 0 pollution. |
...
- 2026-06-21 — **T3c review (reviewer, by running) → ALL ROWS VERIFIED ✅.** Verified each row by running, not on
  ...
  `verify-contract.js` → v4; drift guard **passes on v4 + proven to FAIL on master** (real teeth); `test-cli-exec-gate.mjs`
