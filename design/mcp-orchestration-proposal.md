...
AgentTalk launches each agent by spawning a provider CLI (`claude`, `gemini`, `codex`)
...
1. **Brittle transport.** Agent "actions" are free-text stdout lines interleaved with
   model prose, ANSI noise, and CLI logging. We parse them with prefix matching and
...
4. **Output scraping for side data.** Quota/usage is read by literally scripting the
   CLI UI (`scrapeClaudeUsageViaSlashCommand` drives `/usage` via `expect`). This is
...
in the codebase — just for one provider, in one direction (CLI-as-server,
Node-as-client), bespoke per Codex's schema.

This proposal generalizes that idea and **inverts the common case**: instead of the
CLI being the MCP server, the **Node orchestrator becomes the MCP server** that all
agents connect to.
...
- **Host** — the LLM application (the provider CLI). Embeds an MCP **client**.
...
**Transports:**
- **stdio** — server is a subprocess of whoever spawns it; single client. (This is how
  the Codex path works today; the CLI/Node owns the server as a child.)
...
provider CLIs are MCP _clients_ that connect into it.** This inverts the usual setup —
and the old Codex path, where the CLI was the server and Node the client (see §2).

AgentTalk spawns each CLI and configures it — via a small stdio→WebSocket bridge
(`@fausto/mcp-orchestration`, see §11A) — to dial into AgentTalk's central MCP server.
The model running inside the CLI then **calls AgentTalk's tools** (`send_to_agent`,
...
```
        ...
        tools: send_to_agent, submit_plan, ...   │
                  ▲          ▲          ▲         │
                  │  each CLI connects IN as an MCP CLIENT
...
So "with MCP support" means the CLIs can act as MCP **clients** (dial an external
...
- **Node hosts a single MCP server** over **Streamable HTTP** (a long-lived endpoint
  owned by the orchestrator process, independent of any one CLI's lifecycle).
- **Each spawned CLI is configured to connect to it** via the provider's own MCP
  config (no patching of the CLI — just config + spawn):
...
- **Per-provider MCP support varies.** All three support custom MCP servers, but
  ...
  and must be verified per CLI version.
- **Not a pub/sub bus.** (Rejected Topology B: a standalone MCP server with both the
  CLI and Node as clients. MCP is request/response client→server, not a message bus
...
4. **Provider capability matrix:** confirm per-CLI support for Streamable HTTP,
...
- **Decision:** **Stdio-Bridge-for-All**. Node will host a single, multi-tenant **WebSocket (WS)** server. Every spawned CLI is configured to communicate exclusively via its native, verified **stdio** channel.
- **Bridge Relay:**
  - The orchestrator configures each spawned CLI to run a lightweight `mcp-bridge.js` wrapper as its subprocess.
  - The bridge translates the CLI's stdio JSON-RPC stream directly over a local WebSocket connection back to the central server:
    `"command": "node", "args": ["path/to/mcp-bridge.js", "ws://localhost:<port>/?agentId=<agent_uuid>"]`
  - This eliminates direct CLI HTTP/WS client configuration issues (such as Codex's broken HTTP client initialization or Claude's HTTP-timeout regressions) from our critical path.
...
1. **Gemini Workspace Trust:** The spawn environment configuration for Gemini **must** include `GEMINI_CLI_TRUST_WORKSPACE=true` or pass the `--skip-trust` CLI flag. Otherwise, Gemini silently overrides YOLO mode and disables MCP servers in untrusted directories.
2. **Non-Racy Configuration Cleanup:** Write temporary configurations to unique paths (e.g., `.mcp.<agentId>.json`) and clean them up on process **exit** hooks rather than immediately after spawn, preventing races if a CLI lazy-loads tools.
