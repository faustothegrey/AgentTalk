...
AgentTalk currently drives provider MCPs (Claude, Gemini, Codex) as child processes, communicating via a line-prefixed stdout protocol: `[AgentTalk]:REQ/EVT/RES:{json}`. 
...
graph TD
    ...
    subgraph Provider MCPs
        ClaudeClient[Claude MCP] -->|Stdio JSON-RPC| ClaudeBridge
        GeminiClient[Gemini MCP] -->|Stdio JSON-RPC| GeminiBridge
        CodexClient[Codex MCP] -->|Stdio JSON-RPC| CodexBridge
    end
```

### Key Technical Aspects:
1. **WebSocket Central Server:** Node hosts a central WebSocket server for turn routing.
2. **Stdio Bridge:** All MCPs execute a local bridge wrapper that maps standard stdio streams to WebSocket sockets.
...
We commit to a **stdio-bridge-for-all** topology to keep direct MCP network client implementations out of our critical path:
1.  **Unified WebSocket Server:** Node hosts a single central multi-tenant WebSocket server on a dedicated port.
2.  **Stdio Bridge Subprocess:** The orchestrator configures every launched MCP to run `mcp-bridge.js` as its stdio subprocess:
...
1.  **Gemini Workspace Trust:** Launchs must configure `GEMINI_CLI_TRUST_WORKSPACE=true` or pass `--skip-trust`.
