...
The orchestrator manages agent processes directly using the Node.js `child_process` module. The core of the system is the `ProcessAdapter`, which provides a unified interface for process control.
...
export interface ProcessAdapter {
  launch(id: string, command: string, options?: ProcessLaunchOptions): void;
...
The implementation (`ProcessAdapterImpl`) uses Node's `child_process` module with `shell: true` and streams `stdout`/`stderr` output via push-based `onData` callbacks. Each process can be launched with an optional working directory (`cwd`) and custom environment variables.
...
- **creating**: Agent registered in the registry, process not yet launched.
- **starting**: Process launched, waiting for `READY` packet within a configurable timeout.
