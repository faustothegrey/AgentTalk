# NodePTY Architecture

NodePTY is an orchestrator for LLM-based agents. It manages the lifecycle, communication, and observation of agent processes.
...
- **Agent**: The LLM-driven CLI process (e.g., `agent-cli`) that performs tasks and communicates via the NodePTY protocol.
...
3. The orchestrator scans the output for protocol lines (prefixed with `[NodePTY]:`).
...
| Feature | cmux-hosted (Deprecated) | NodePTY (Current) |
| :--- | :--- | :--- |
