...
- **Requirement:** Prevent configuration conflicts across concurrent agent processes by spawning them with unique configuration directories, cleaning up files recursively upon exit.
- **Resolution:**
  - `ClaudePersistentExecutor` writes settings to a custom `mkdtempSync` path. To circumvent Claude CLI ignoring the config path inside `CLAUDE_CONFIG_DIR`, we pass the configuration path explicitly via `--mcp-config` and `--strict-mcp-config` CLI arguments.
...
- **Verification:**
  - Unit tests in `apps/orchestrator/src/__tests__/executor-runtime.test.ts` assert that `mcp.json` is created, `--mcp-config` and `--strict-mcp-config` are appended to Claude Code's spawn arguments, and temp folders are cleaned up on close.
  - Independent verification spikes prove Gemini and Claude CLI connections function correctly with this setup.
...
- **Resolution:** Gemini spawns include the environment setting `GEMINI_CLI_TRUST_WORKSPACE: 'true'`.
