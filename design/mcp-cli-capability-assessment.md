# Assessment: Do the Provider CLIs Satisfy the MCP Transport Requirement?
...
> *"Provider capability matrix: confirm per-CLI support for Streamable HTTP, sampling,
> and resource subscriptions."*
> It is the highest-risk item in the proposal because the entire Topology A design
> (Node-as-MCP-server, agents connect as clients) assumes all three CLIs can act as
...
- **Gemini CLI** — clears both, with one open HTTP-connect bug to verify and a 10-min
  default tool timeout.
- **Codex CLI** — Streamable HTTP is broken in practice (needs a stdio bridge). Over
...
So the proposal's universal "single HTTP/SSE server that all CLIs connect to" assumption
does not hold across all three providers.

---

## Current spawn reality in the repo (baseline)

The CLIs are invoked from `PATH` (unpinned) with non-interactive flags:
...
## Sub-requirement (a): CLI as a client of a *custom HTTP/SSE* MCP server

| CLI | HTTP/SSE client support | Reality check |
|---|---|---|
| **Claude Code** | ✅ Yes | `claude mcp add --transport http`/`sse`; also `ws`. Mature. |
| **Gemini CLI** | ✅ Yes | `httpUrl` = HTTP streaming, `url` = SSE, `command` = stdio (transport chosen by which key is set). Open bug [#5268](https://github.com/google-gemini/gemini-cli/issues/5268) "Streamable HTTP cannot connect" — verify on the installed version. |
| **Codex CLI** | ⚠️ On paper, **broken in practice** | `config.toml` supports `url` + `bearer_token_env_var` (+ `http_headers`, `env_http_headers`), presented as standard (not experimental). But [openai/codex #11284](https://github.com/openai/codex/issues/11284) (v0.98.0, still open) reports HTTP servers fail to initialize / show "no tools available" while the *same servers work in Claude Desktop & Cursor*. Documented workaround: a transport bridge (e.g. supergateway) converting HTTP→stdio. |
...
waiting on another agent's turn, slow I/O). So the question is how long each CLI will
hold a `tools/call` open before timing out. Default per-tool-call timeouts differ by an
**order of magnitude**:

| CLI | Default tool-call timeout | Verdict |
|---|---|---|
| **Claude Code** | `MCP_TOOL_TIMEOUT` default **~28 h** when unset | ✅ Generous. |
| **Gemini CLI** | **600,000 ms (10 min)**, per-server `timeout` configurable; `trust: true` bypasses tool confirmations | ✅ Workable; raise the per-server `timeout` if handlers can exceed 10 min. |
| **Codex CLI** | **Hard ~120 s default** tool-call timeout (`timed out awaiting tools/call after 120s`), confirmed in the Phase-1 spike on 0.133.0. 70 s passed because it was under the ceiling; **180 s fails at default** and succeeds only with `tool_timeout_sec` raised. | ✅ RESOLVED: the doc-cited "60 s" is wrong, but a real **120 s** ceiling exists. Set `mcp_servers.<name>.tool_timeout_sec` (e.g. ≥600) for any block >120 s. With it raised, **180 s verified over the WS bridge**. |
...
- **Gemini CLI:** satisfies (a) (one open HTTP bug to verify) and (b) within a
  ...
  don't load; spawns must set `GEMINI_CLI_TRUST_WORKSPACE=true` / `--skip-trust` (see
  Independent Reproduction).
- **Codex CLI:** HTTP initialization broken (needs stdio bridge). Over **stdio** it is
...
1. **Run a capability spike** that, for each installed CLI version, verifies: custom
   HTTP/SSE connect, and a tool handler that blocks for a realistic handler window
   returns successfully. Record exact CLI versions tested.
...
1. **Codex CLI (0.133.0) - Stdio MCP & 70s Blocking Tool Call:**
   - **Method:** Configured purely dynamically at spawn time using `-c 'mcp_servers.test-server.command="node"'` and `-c 'mcp_servers.test-server.args=["..."]'`.
...
3. **Gemini CLI (0.41.2) - Project-Scoped Stdio MCP:**
...
- **Zero Configuration Pollution:** Instead of editing global config files, we will dynamically configure Codex via `-c` options, and write temporary `.mcp.json` / `.gemini/settings.json` files for Claude and Gemini just before spawning, deleting them immediately after the process starts.
...
2. **The central-server + bridge + routing was never tested.** stdio means the MCP server
   is a **child of each CLI**, not the central Node orchestrator. Keeping the centralized
...
4. **"Delete temp config immediately after the process starts" is racy.** It assumes the CLI
...
reproduced** — each CLI connected, executed a **70s** blocking call, and returned the
...
2. **Gemini requires workspace trust — not mentioned in the original spike.** In an
   ...
   **Operational consequence:** the orchestrator spawns agents in arbitrary working dirs,
   so the trust env var/flag is a **required** part of the Gemini spawn config, not
   optional.
...
WebSocket MCP server + assertions) and `scripts/mcp-bridge.mjs` (stdio↔WS relay). Each CLI
spawns the bridge as its stdio MCP server; the bridge relays JSON-RPC over WS to the
...
- **Bridge routing works.** CLI → stdio → `mcp-bridge.mjs` → WS → server: handshake +
  `tools/list` + `tools/call` all route correctly for all three providers. The
  stdio-bridge-for-all topology is validated end-to-end with real CLIs.
...
1. **Bug Found & Fixed in Concurrency Wall-Time Assertion:**
   - The original wall-time check (`wallSec < BLOCK_SECONDS * 2 - BLOCK_SECONDS / 2`) was too strict for short blocks (like 5s) because it did not account for the non-trivial startup latency of Codex and Claude Code CLI processes on macOS (approx. 5-8 seconds).
