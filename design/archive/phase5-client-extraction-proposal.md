...
| A4 | **MCP-failure surfacing** — distinguish transport-drop vs clean `terminated` vs real MCP/exec failure, and surface the cause instead of collapsing to a generic close. | **[RESOLVE]** | Carry-forward open item from Milestone 04. This is channel/exec robustness, **not** workflow. |
...
| A6 | **Keepalive vs provider idle budgets** — 20 s ping must keep MCPs under their idle ceilings (codex ≤120 s). | **[NOTE]** | Make `pingIntervalMs` driven by the capability descriptor, not a hardcode. |
...
MCPs but **none** of the orchestrator/consensus code.
...
- **"Zero-dependency" is misleading.** The client *launchs the provider MCPs*
  (codex/claude/gemini), which must already be installed. The heavy prerequisite is the
  provider MCP, not the Node runtime — bundling Node does not remove it.
...
- No hard cutover; legacy/auto-launch paths stay behind their flag, behavior preserved.
...
2. Then Workstream A robustness: **A1** (reconnect+backoff), **A2** (re-attach semantics),
   **A3** (version negotiation), **A4** (MCP-failure surfacing).
...
2. **Workstream A (hardening) — implementation backlog** (design settled): A1 reconnect +
   ...
   negotiation, A4 MCP-failure surfacing; A5–A7 as [NOTE].
...
- **[BLOCK] B5 — Drop the last shared dependency (`@fausto/mcp-orchestration`).** The worker
  ...
  launched MCP at the stdio↔WS bridge script. **Vendor the bridge script into
