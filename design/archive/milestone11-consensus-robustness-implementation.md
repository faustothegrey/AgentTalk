...
> **Plan:** `design/archive/milestone11-consensus-robustness-plan.md`
> **Opened:** 2026-06-30 (SM Hermes, PO Fausto confirmed)
> **Old plans:** `design/archive/milestone10-protocol-compliance-plan.md` (thesis), `design/archive/milestone10-phase2-plan.md` (T1–T4 breakdown)
...
| **M11-T3** | Turn-budget / Referee (bound discussion, force-advance on non-convergence) | **DONE** ✅ merged to master |
...
- `git diff --stat 94d7a8a..04edc8a` → one docs-only file:
  `design/archive/milestone11-consensus-robustness-implementation.md` (`28` lines changed). No production code changed.
...
- `git show --stat --oneline 04edc8a` → docs-only MT2 completion commit: one file,
  `design/archive/milestone11-consensus-robustness-implementation.md`.
...
- Read `design/archive/milestone11-consensus-robustness-plan.md` and checked cited files/line ranges with `wc -l` / `nl -ba`.
- `git diff --check -- design/archive/milestone11-consensus-robustness-plan.md design/archive/milestone11-consensus-robustness-implementation.md` → clean.
...
Evidence run/read:
- Re-read `design/archive/milestone11-consensus-robustness-plan.md` after corrections.
...
- `git diff --check -- design/archive/milestone11-consensus-robustness-plan.md design/archive/milestone11-consensus-robustness-implementation.md`
  → clean.
...
Reviewed: `design/archive/m11-t1-consensus-respond-task-breakdown.md`.
...
- `git diff --check -- design/archive/m11-t1-consensus-respond-task-breakdown.md` → clean.
- `git diff --no-index --check /dev/null design/archive/m11-t1-consensus-respond-task-breakdown.md` → no output.
- `LC_ALL=C rg -n "[^\x00-\x7F]" design/archive/m11-t1-consensus-respond-task-breakdown.md` → no output.
...
- `rg -n "git diff --check|git -C ../agentalk-mcp-client|../agentalk-mcp-client/wire-contract.json"
  design/archive/m11-t1-consensus-respond-task-breakdown.md` shows the invalid cross-repo diff-check command has been split:
...
- `rg -n "attach-skill.md|submit_plan|consensus_respond\(action,payload\)"
  design/archive/m11-t1-consensus-respond-task-breakdown.md` confirms `/Users/fausto/Software/agentalk-mcp-client/attach-skill.md:5-10`
...
- `rg -n "test-mcp-provider|test-mcp-gate|test-live-gate|active live MCP gate"
  design/archive/m11-t1-consensus-respond-task-breakdown.md` confirms `scripts/test-mcp-provider.mjs:12-109` is now covered.
...
- `rg -n "unless the reviewer|reviewer explicitly approves|compatibility shim|PO-approved rescope|old planning tool names are no longer accepted"
  design/archive/m11-t1-consensus-respond-task-breakdown.md` confirms the reviewer-only compatibility-shim escape hatch is gone;
  old planning tools are not accepted, and any compatibility shim requires explicit PO-approved rescope.
- `git diff --check -- design/archive/m11-t1-consensus-respond-task-breakdown.md design/archive/milestone11-consensus-robustness-implementation.md`
  → clean.
- `git diff --no-index --check /dev/null design/archive/m11-t1-consensus-respond-task-breakdown.md` → no output.
- `LC_ALL=C rg -n "[^\x00-\x7F]" design/archive/m11-t1-consensus-respond-task-breakdown.md` → no output.
...
- `git status --short --branch` in AgentTalk → branch `m11-t1-consensus-respond` with dirty
  `design/archive/milestone11-consensus-robustness-implementation.md` (this verdict) and pre-existing
...
Reviewed: `design/archive/m11-t2-active-reprompting-task-breakdown.md`.
...
- `git status --short --branch` → `## master...origin/master` plus only the untracked
  `design/archive/m11-t2-active-reprompting-task-breakdown.md`.
- `git diff --check -- design/archive/m11-t2-active-reprompting-task-breakdown.md` → clean.
...
- `git diff --check -- design/archive/milestone11-consensus-robustness-implementation.md packages/runtime-core/src/registry/__tests__/team-protocol-correction.test.ts packages/runtime-core/src/registry/team-coordinator.ts`
...
Reviewed: `design/archive/m11-t3-turn-budget-referee-task-breakdown.md`.
...
- `git status --short --branch` → `## master...origin/master` plus only untracked
  `design/archive/m11-t3-turn-budget-referee-task-breakdown.md`.
- `git diff --check -- design/archive/m11-t3-turn-budget-referee-task-breakdown.md` → clean.
- `wc -l` confirmed cited files/ranges exist:
  `design/archive/m11-t3-turn-budget-referee-task-breakdown.md` has `303` lines,
...
- `rg -n "419-523|525-600|team-coordinator\\.ts:" design/archive/m11-t3-turn-budget-referee-task-breakdown.md design/archive/milestone11-consensus-robustness-plan.md`
  shows `:419-523` and `:525-600` appear only in the task breakdown, not in the approved M11-T3 production scope.
- `git diff --no-index --check /dev/null design/archive/m11-t3-turn-budget-referee-task-breakdown.md` → no output.
...
1. **Production scope is wider than the approved M11-T3 scope.** The plan lists
   ...
   `design/archive/milestone11-consensus-robustness-plan.md:206-214` or the Hermes baton. The approved counting surface is
...
Reviewed: `design/archive/m11-t3-turn-budget-referee-task-breakdown.md` after the scope correction.

Evidence run/read:
- `git rev-parse --short HEAD` → `6038b12`.
- `git status --short --branch` → `## master...origin/master` with modified
  `design/archive/milestone11-consensus-robustness-implementation.md` and untracked
  `design/archive/m11-t3-turn-budget-referee-task-breakdown.md`.
- `git diff --check -- design/archive/m11-t3-turn-budget-referee-task-breakdown.md design/archive/milestone11-consensus-robustness-implementation.md`
  → clean.
...
- `rg -n "419-523|525-600|Read-only context|Approved edit surfaces|Required location|validateProtocolStep|615-880|1910-1963|force|ejectPlanner|late-message|wire" design/archive/m11-t3-turn-budget-referee-task-breakdown.md`
...
3. **Required live observation is missing from the branch evidence.** The approved milestone plan requires a live
   ...
   (`design/archive/milestone11-consensus-robustness-plan.md:221-237`). I found no new task-specific live harness or M11-T3 live
