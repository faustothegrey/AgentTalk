...
**Status:** **🎉 M07 CLOSED — T4b-3 ALL ROWS 3.1–3.5 VERIFIED ✅ (reviewer, by running, round 2) + MERGED both repos.** The semantic path is retired: harness = pure transport + exec (`llm-agent.mjs` 899→195, semantic handlers + `lib/{conversation-runtime,response-schema}.mjs` gone, `handleExecRpc` + a real round-trip test kept); orchestrator no-driver/attach path removed (provider-less → rejected); brainstorm retired end-to-end (pure-brainstorm deletions, `-W`-verified); flagship migrated to mcp. Verified by running: AgentTalk `tsc -b` 0 + vitest **165/165**, harness eslint clean + vitest **1/1**, **both** live tests reach completion (0 pollution), `wire-contract.json` **byte-identical**. Round 1 was sent back for IP-1 (green-by-subtraction: deleted all harness tests + `--passWithNoTests`) and IP-2 (false live-bar deferral); round 2 fixed both. Minor follow-ups logged: IP-3 (report mischaracterised the no-driver change as pre-existing) + IP-4 (no test pins the new rejection) → backlog. **All M07 tasks DONE/merged. ➡️ NEXT: M08 (failure-modes milestone).**
<br>**Prior:** **T3c — ALL ROWS T3c.1–T3c.5 VERIFIED ✅; MERGED** (AgentTalk `fc1c779` / harness `58ef46d`). Contract bumped v3→4 (`submit_exec_result`; hash `97abf6…`; byte-identical both repos); drift guard has teeth; v4 handshake verified live. **T3b-2 — VERIFIED ✅ + MERGED** (AgentTalk `534b4ef` / harness `c15d7c7`): worker run-to-completion via exec-RPC. T3b-1 DONE/merged `ff9296d`; T3a + spikes T3-S1/S2 DONE; D5 set; T1+T2 DONE/merged. M06 closed.
**Plan:** `design/archive/milestone07-centralized-brain-plan.md` (architect-owned; this doc tracks status only).
**Last verified:** 2026-06-22 (T4b-3 round 2, post-merge) · **Verifier:** Claude
...
| **M07-T3** | **MCP harness inversion** (exec-RPC) + reconnect/effect-fence + contract bump | `m07-t3a-mcp` (T3a) | **T3a/T3b DONE+merged ✅**; T3-S1/S2 spikes DONE; **T3c implementation-ready** (rows below) |
...
| ↳ **M07-T3-S2** | **Spike:** prove no-resend (latest-turn-only) is correct + cheap on the mcp path | (reviewer-run, no branch) | **DONE ✅** (no-resend proven; S2 block below) |
| ↳ **M07-T3b-1** | **No-resend for mcp** (latest-turn prompt) + recovery fallback; API path untouched (D5) | `m07-t3b1-no-resend` | **DONE ✅** (merged `ff9296d`; T3b-1.1–1.7 VERIFIED) |
...
| ↳ **M07-T4a** | **Verify mcp multi-agent consensus** (de-risk before deleting) | `m07-t4a-mcp-consensus` | **DONE ✅** (merged `87ebc52` + hotfix `14388cd` for FIND-T4a-2; all rows VERIFIED) |
| ↳ **M07-T4b** | **Retire the semantic path** — RE-SCOPED (pre-flight): production UI rides the semantic harness → migrate all 3 providers to mcp first | (3 stages below) | **RE-SCOPED → 3 stages** (plan §12b) |
| ↳ **M07-T4b-1** | **Verify mcp for claude + codex** (only gemini proven) | `m07-t4b1-mcp-providers` | **DONE ✅** (merged `8e5dfda`; claude + codex live exec-RPC VERIFIED, 0 pollution, 163/163) |
| ↳ **M07-T4b-2** | **Migrate the production/UI path to mcp** (gemini/claude/codex → driver) | `m07-t4b2-migrate-production` | **DONE ✅** (merged `3672f9c`; production team→`completed` via server API, 3/3, both M08 fixes live; needed 2 M08 engine fixes + a test-harness fix). |
| ↳ **M07-T4b-3** | **Delete semantic harness + retire brainstorm + migrate flagship** | `m07-t4b3-retire-semantics` (both repos) | **DONE ✅** (merged — AgentTalk + harness; rows 3.1–3.5 VERIFIED by running, round 2). Harness = pure transport+exec; no-driver path removed; brainstorm retired; flagship → mcp. **Closed M07.** Round 1 sent back for IP-1/IP-2; round-2 follow-ups IP-3/IP-4 → backlog. |
...
`TeamCoordinator`, the MCP harness, or the attach/single-agent paths** (plan §10 guardrails).
...
## Task M07-T3 — MCP harness inversion (exec-RPC)  *(T3a SPEC READY — branch `m07-t3a-mcp`)*
...
| **T3a.4** **Flag/config (coexistence, D1):** `provider:'mcp', providerName:'gemini'` takes the exec-RPC path; default behaviour unchanged / off by default. **Mocked CI test** drives a planner turn end-to-end via the driver+McpCompleter. | **done** | **VERIFIED ✅ (round 2)** | Coexistence routing was already sound (driver started for `provider:'mcp'` with a `McpCompleter`; `await_turn` pulls `awaitExecTurn` only for mcp; default/other providers unchanged + off by default — proven live). Round-2 closed the two round-1 gaps: the **mocked CI test now passes** (see T3a.2) and the **debug `console.log` in `await_turn` is removed** (`grep "await_turn called for"` → gone). |
| **T3a.5** **Live:** one real **agy** planner turn via exec-RPC, recorded (log/transcript). | **done** | **VERIFIED ✅ (reviewer ran it)** | "Script exists" ≠ recorded run — so the reviewer **ran it**: `node scripts/test-mcp-gate.mjs` launched the real harness → real **agy** → `exec_rpc{prompt:"Say hello"}` → agy returned `"Hello! How can I help you today?"` → `submit_exec_result` (raw) → driver translated → `send_to_agent` → **TEST PASSED**, exit 0. Recorded: **`m07-t3a-mcp-smoke.log`**. |
| **T3a.6** **No regression:** full suite green; M05/M06 attach + API (T1/T2) paths unchanged; `tsc -b` clean **committed**. | **done** | **VERIFIED ✅ (round 2)** | Round-1 REFUTED (suite red + uncommitted). **Round 2:** ran `npx vitest run` → **157/157 green (26 files)**; `tsc -b` clean (exit 0). **Now committed** in both repos — orchestrator `3951db9`+`40dd419` on `m07-t3a-mcp`; harness `782cbe7` ("Handle exec-RPC in client"); both working trees clean. T1/T2 + M05/M06 attach paths unchanged (157 includes the full consensus/driver suite). |
...
1. **Steady state → option 2 (native session).** For mcp agents, stop resending the transcript: the
   driver should send **only the latest turn** and rely on the MCP's native `--continue` (proven to work,
   S1.2b). This is the O(1)-prompt, context-limit-safe path. **Requires:** a mcp branch in the
...
2. **Recovery → option-1 fallback (brain re-seeds memory).** Native session does **not** survive a harness
   ...
   works today). So the safety net is free: on a lost/unrecoverable session, **rebuild a fresh MCP session
...
O(n) → context blow-up on long tasks. **Goal:** prove a mcp agent can run with **no resend** (latest
turn only, native `--continue` memory) and stay correct, and quantify the saving. **mcp only** (API
...
| **Approach for T3b.** | **RECOMMEND** | Confirms D2 #1: add a **mcp branch in the driver's prompt-building** that sends only the latest turn (native memory carries context). Guard it — API/T1-T2 resend path stays byte-for-byte. Pair with the LB-5 recovery fallback when productionised. |

**Net:** removing the resend is **proven safe and worth it** — flat prompt, correct on native memory. T3b
should implement the no-resend mcp path as a **priority** (it's the cost win), with the recovery
...
- **IMP-T3b-1 (reconnect+exec-RPC delivery gap).** In probe 1, the turn issued **after** a harness
  ...
  handling (this gap + the effect-fence D4 + `McpCompleter` disconnect-rejection) is deferred to the
  parked failure-modes milestone (see `backlog.md`). Not a T3-S1 defect (the spike's job was the session question).

## Task M07-T3b-1 — no-resend for mcp + recovery fallback  *(SPEC READY — branch `m07-t3b1-no-resend` off `master`)*

### T3b-1: No-resend implementation (completed)
Implement the `mcp` no-resend prompt builder within the orchestrator's driver loop.
*   [x] **T3b-1.1** — Extend `Completer` interface with `maintainsSession?: boolean`. Set to `true` for `McpCompleter`, `false` for `ApiCompleter`.
...
|---|---|---|---|
| **T3b-1.1 — Completer capability flag.** Add `maintainsSession?: boolean` to the `Completer` interface; `McpCompleter`=true, `ApiCompleter`=false (D5). | **done** | **VERIFIED ✅** | `completer.ts`: flag on interface; `ApiCompleter=false`, `McpCompleter=true`. Correct per D5. |
...
crash/failure handling (effect-fence D4, exec-RPC reconnect IMP-T3b-1, `McpCompleter` disconnect-rejection)
...
> (`McpCompleter` never rejects). Acceptable because the exec path is **flag-gated / off by default** (D1);
...
|---|---|---|---|
| **T3b-2.1 — Worker via exec-RPC.** Route the worker turn (`team_work_assign`) through the mcp completer path (same `exec_rpc`), so the worker is driven by the orchestrator brain — not the M05/M06 semantic harness. | done | **VERIFIED ✅ (core idea only)** | Routing is sound: `handleTeamWorkAssign` → `executeApiPrompt(...,execOpts)` → completer; mcp worker goes through `exec_rpc`. Correct — but ships with violations B1–B5 that block the task. |
| **T3b-2.2 — Worktree workdir.** The worker exec runs in a **per-task `git worktree`** via `AGENTTALK_WORKDIR`; orchestrator sets it; harness runs agy there. Define worktree create (cleanup is best-effort / out of scope failure-side). | done | **REFUTED ❌ (B1, B4)** | Worktree IS provisioned — but **in `TeamCoordinator` (spec: DO NOT TOUCH — B1)**, via an **unconditional `execSync('git worktree add')`** that runs for **every** worker (not gated to mcp) and **fired during the test suite → 8 real worktrees + `task-task-*` branches** created in the repo (B4; reviewer pruned them). |
...
| **T3b-2.4 — Mocked CI test.** A mcp worker turn end-to-end (mocked transport/exec): `team_work_assign` → exec_rpc → `submit_exec_result` → `submit_work_response`/`submit_work_result`. Deterministic. | done | **PARTIAL ⚠️** | Test passes in vitest, but **accesses the private `teamCoordinator` member → 2 of the tsc errors (B3)**, and can trip the real-worktree `execSync` (B4). Not hermetic. |
| **T3b-2.5 — Live.** One real **agy** worker turn via exec-RPC that makes a small, real change inside a worktree, recorded. | done | **VERIFIED ✅ (reviewer, by running)** | Ran `scripts/archive/m07-t3b2-live-worker.mjs` (mcp worker + externally-launched agy harness). Orchestrator provisioned `/tmp/agentalk-task-<id>` and sent `exec_rpc` with `cwd=<that worktree>`, `timeoutMs=600000`; real agy ran (~30s), returned valid `work_accept`, and **wrote `WORKER_PROOF.txt` with the exact expected bytes** (`agy-was-here-<taskId>\n`) — verified by reading the file, not by trusting agy's claim. Recorded: `m07-t3b2-live-worker.log`. Cleanup verified: 0 orphan worktrees/branches/`/tmp` dirs after. **Observation (FIND-T3b2-1, nested worktree):** the worker *prompt* still says "use strictly `git worktree`," so agy — **already** inside the orchestrator's worktree — created a **nested** worktree (`./worker-worktree`) and wrote there. DoD met (change *is* inside a worktree), but the change lands one level deeper than the orchestrator's `cwd`. Pre-existing prompt behavior; flagged for the worker-prompt cleanup (backlog), not fixed here. |
...
provisioning moved into the driver, gated `if (this.completer.maintainsSession)` (mcp only). **B2** harness
...
### T3b-2 — 2.5 LIVE now VERIFIED (reviewer, by running) — ALL ROWS GREEN, ready to merge
2026-06-21: Ran the live agy worker turn (`scripts/archive/m07-t3b2-live-worker.mjs`): exec-RPC round-trip to a real
agy worker with `cwd=<per-task worktree>`, agy wrote `WORKER_PROOF.txt` with the **exact expected bytes**
(verified by reading the file), recorded to `m07-t3b2-live-worker.log`, repo clean after (0 orphan worktrees/
branches). No-regression re-confirmed this session: **`tsc -b` exit 0** + **vitest 160/160** (4.05s). ⇒ **all
rows 2.1–2.6 VERIFIED ✅ — T3b-2 ready to merge (AgentTalk + agentalk-mcp-client).** Surfaced **FIND-T3b2-1**
(agy nests its own worktree inside the orchestrator's, because the worker prompt still mandates `git worktree`)
→ worker-prompt cleanup parked in backlog; not a blocker.
...
- **B1 — Touched `TeamCoordinator` (spec: DO NOT TOUCH).** Worktree provisioning + `cwd`/`timeoutMs` added to the
  ...
  **Fix (directive):** revert `TeamCoordinator` to master (byte-identical). Provision the worktree **in the
  mcp worker path only** — inside the driver's `handleTeamWorkAssign` (or a helper) **when
  `completer.maintainsSession`** — gated to mcp, engine untouched. Keep `git worktree add` **out of the test
...
provider defaults change (mcp stays opt-in). `exec_rpc` (an EVT subtype) and `cwd`/`timeoutMs` (payload fields)
...
|---|---|---|---|
| **T3c.1 — Bump orchestrator contract.** Add `submit_exec_result` to `data.mcpTools` in `packages/contracts/wire-contract.json`; `version` 3→4; recompute `hash = sha256(JSON.stringify(data, null, 2))`. | done | **VERIFIED ✅ (by running)** | `verify-contract.js` prints "verified successfully (v4)". **Independently recomputed** the hash (`sha256(json.dumps(data,indent=2))`) = `97abf6…89a5` — matches the file. Diff = exactly `+submit_exec_result` + version 3→4; `exec_rpc` absent from the contract (as specced). |
| **T3c.2 — Bump harness contract in lockstep.** Apply the **byte-identical** v4 file to `agentalk-mcp-client/wire-contract.json`. | done | **VERIFIED ✅ (by running)** | `diff -q` between the two copies → IDENTICAL (no output); harness copy hash field = `97abf6…89a5`. |
| **T3c.3 — Drift guard (recommended: a permanent test).** Assert the contract's `mcpTools` equals the real registered `AGENTTALK_MCP_TOOLS` set — add a small vitest so this exact drift (a tool registered but absent from the contract) can't recur. | done | **VERIFIED ✅ (by running)** | `mcp-tools.test.ts` (Set-equality) **passes on v4**; **proven to FAIL on `master`** (recomputed against `git show master:…` → unequal, missing `submit_exec_result`) ⇒ real teeth (fail-before/pass-after). Runs in the suite (28 files). |
| **T3c.4 — Handshake no-regression.** Happy path: a mcp agent attaches + completes a turn with v4 on **both** sides (re-run `scripts/test-mcp-gate.mjs`). Negative path: a v3-client / v4-server mismatch is **rejected** (mocked/unit on `McpServer`). | done | **VERIFIED ✅ (by running)** | Ran `test-mcp-gate.mjs` **live** → exit 0: mcp agent attached (v4 handshake accepted — no rejection), agy ran, `submit_exec_result` + `send_to_agent` round-tripped. Negative: `mcp-server.test.ts` asserts error `-32000` + "Contract hash mismatch" + WS close `1008` — passes. |
...
mcp/T3). **Not dead-code removal:** the M06 flagship `test-live-gate.mjs` still rides the **semantic attach path**
...
### T4a — verify mcp multi-agent consensus  *(IMPLEMENTATION-READY — branch `m07-t4a-mcp-consensus` off `master`)*
Prove the brain replaces the semantic harness for **consensus** before deleting it. Same `InProcessAgentDriver` as API/T2
(only `McpCompleter` swapped) ⇒ low architectural risk, but **unproven live**. agy is protocol-fit ⇒ not frozen.

| T4a DoD item | Implementer claim | Reviewer verdict | Evidence |
|---|---|---|---|
| **T4a.1 — Live mcp consensus gate.** New script (mcp analog of `test-live-gate`): 2 mcp **agy** planners + 1 mcp worker, server-side brain, **full flow** fact_collection → discussion → proposal → acceptance → submit_plan → confirm → worker exec → `team_task` **completed**, all via exec-RPC. **Recorded** (log). | done | **VERIFIED (functional) ⚠️ — FIND-T4a-1 blocks merge** | Ran `scripts/test-live-mcp-team.mjs` **live, exit 0** (`m07-t4a-live-consensus.log`). Tool-call order proves a **genuine full protocol** (both planners `ack_planning_protocol`→`fact_collection_end`→`send_to_agent`→`agreement_proposal`(a)→`agreement_acceptance`(b)→`submit_plan`→worker `submit_work_response`/`submit_work_result` → **completed**); all agents `provider:'mcp'` (driver path, not attach). Minor: planner-b sent a redundant late `submit_plan` (harmless). **FIND-T4a-1 (cleanup) — round 1 path-scoped fix REFUTED (leaked a sibling agy worktree); round 2 snapshot-diff fix NOW VERIFIED ✅** — reviewer re-ran the gate **twice** (agy nondeterministic); both exit 0 + **zero pollution** after each (worktrees/branches/`/tmp` all clean). No-regression re-confirmed: `tsc -b` 0, vitest **163/163**. ⇒ **all T4a rows VERIFIED — ready to merge** (AgentTalk only; harness untouched). |
| **T4a.2 — Deterministic CI test.** Mocked mcp consensus (mock `McpCompleter`/exec transport, à la T2.3): full flow → `awaiting_confirmation` → `completed`. No live calls. | done | **VERIFIED ✅ (by running)** | `team-mcp-consensus.test.ts` drives the **real** engine (mcp agents → `InProcessAgentDriver`+`McpCompleter`, scripted mock harness loop) to `completed`. **Deterministic: 3/3 isolated runs** + green in the full suite. Not a trivial stub. |
...
> and `registry.activateAgent` drives only `api`/`mcp` (`registry.ts:210`) — so **production MCP agents run on the
> semantic harness over the wire.** Deleting it would break the live product. And **mcp is proven only for gemini
> (agy)**; claude/codex via exec-RPC is unproven. ⇒ migrate **all three** providers to mcp (Fausto) **before** deleting,
...
> T4b-1/T4b-2 live gates need the **claude + codex MCPs authed locally** (see Op notes).

**Stage T4b-1 — verify mcp for claude + codex**  *(IMPLEMENTATION-READY — branch `m07-t4b1-mcp-providers` off `master`)*

| T4b-1 DoD item | Implementer claim | Reviewer verdict | Evidence |
|---|---|---|---|
| **T4b-1.1 — Live mcp claude turn.** Parameterize `test-mcp-gate.mjs` (or a sibling) for `provider:'mcp', providerName:'claude'`: one real **claude** MCP turn via exec-RPC round-trips (`exec_rpc`→`submit_exec_result`→`send_to_agent`). Recorded. **Clean up** any worktrees ([[LB-9]]). | done | **VERIFIED ✅ (by running)** | Ran `scripts/test-mcp-provider.mjs claude` **live, exit 0** (`m07-t4b1-claude.log`): real claude reply over exec-RPC — `send_to_agent payload: "Hello! What can I help you with today?"`. **0 worktrees / 0 branches** after (snapshot-diff cleanup present). |
| **T4b-1.2 — Live mcp codex turn.** Same for `providerName:'codex'` — one real **codex** MCP turn via exec-RPC. Recorded. | done | **VERIFIED ✅ (by running)** | Ran `…provider.mjs codex` **live, exit 0** (`m07-t4b1-codex.log`): real codex reply over exec-RPC — `send_to_agent payload: "Hello.\n"`. **0 pollution** after. ⇒ mcp now proven for **all 3** providers (gemini/claude/codex). |
...
**Stage T4b-2 — migrate the production/UI path to mcp**  *(SPEC READY — branch `m07-t4b2-migrate-production`; after T4b-1)*

| T4b-2 DoD item | Implementer claim | Reviewer verdict | Evidence |
|---|---|---|---|
| **T4b-2.1 — Route production providers to the driver.** UI-created `gemini`/`claude`/`codex` agents get the mcp `InProcessAgentDriver` (exec-RPC), not the wire/semantic path — at `registry.activateAgent` and/or the server create/start path. Define the mapping; keep `api` untouched. | done (Gemini) | **VERIFIED ✅ (by running)** | `registry.ts:210` gate now includes `gemini`/`claude`/`codex`; live log shows `Starting InProcessAgentDriver for gemini-backed agent` for all 3, and the whole consensus runs via `exec_rpc` (not semantic EVTs). `api` untouched. |
| **T4b-2.2 — UI still works (behavior preserved).** Prove a real agent turn **and** a team/consensus turn through the migrated path (the web UI / server API end-to-end). Recorded. | done (Gemini) | **VERIFIED ✅ (by running, reviewer)** | Ran `scripts/test-live-server-api-team.mjs` (server API → migrated mcp drivers): **2-planner + worker team reaches `completed`** (`submit_plan` → confirm → `team_work_assign` → `submit_work_result`). **Both M08 fixes fired live.** **Gemini's first "pass" did NOT reproduce** — the *test* confirmed the global newest planning run, not THIS task (536 stale files) → worker never delegated → timeout. Reviewer fixed the test (target task by id, check confirm response, 6-min budget, LB-9 cleanup, `return`-not-`exit`): **3/3 PASS, worker completes, 0 pollution** (`b305732`). |
| **T4b-2.3 — No regression.** Full suite + `tsc -b` green, committed; existing api/mcp/attach tests still pass (attach not yet deleted). | done (Gemini) | **VERIFIED ✅ (by running)** | `tsc -b` exit 0; vitest **166/166**; post-suite **0 worktrees / 0 branches**; tests hermetic (proper `vi.mock`, no `'custom'` dodge). |

**Stage T4b-3 — delete semantic path + retire brainstorm + migrate flagship**  *(IMPLEMENTATION-READY — branch `m07-t4b3-retire-semantics`, BOTH repos; the LAST M07 piece)*

> **Pre-flight (2026-06-22, by reading code — grounds the inventory below):**
> - **Harness deletion is SURGICAL, not "rm 2 files".** In `agentalk-mcp-client/llm-agent.mjs` the semantic code spans
>   ~L85–728: imports (`conversation-runtime` L7, `response-schema` L13–17), the module-level `conversationRuntime`
>   (L85), and the handlers `handleFactCollectionBegin`(394)/`handleTeamTaskAssign`(453)/`handleTeamWorkAssign`(561)/
>   `handleBrainstormStart`(667)/`handleBrainstormMessage`(697) + the structured-response message handling that calls
>   `parseStructuredResponse`/`buildPrompt`/`extractSystemRequiredCall`. **`handleExecRpc` (L729+) is semantics-free and
>   uses none of it** → it + transport survive. So: excise L85–728's semantic bits + the two imports + delete
>   `lib/conversation-runtime.mjs` + `lib/response-schema.mjs`; trim the `handleInboundEvent` dispatch (L769) to the kept events.
> - **KEEP the orchestrator's event EMISSION.** The in-process driver *consumes* `conversation_start`(IP-driver L85),
>   `fact_collection_begin`(L99), `team_work_assign`(L104) server-side and routes via `exec_rpc`. T4b-3 deletes the
>   **harness** consumers + the **no-driver wire path**, NOT the orchestrator's emission (that's the engine — leave it).
> - **No-driver path to remove:** `activateAgent` in `packages/runtime-core/src/registry/registry.ts` returns early for `api|mcp|gemini|claude|codex` (driver);
>   the fall-through after that `return` is the attach/wire path → remove it; a provider-less/unknown agent is rejected.
> - **Brainstorm reach (exact, repo-relative paths):** `packages/runtime-core/src/registry/team-coordinator.ts` (37),
>   `packages/runtime-core/src/registry/registry.ts` (4), `packages/contracts/src/protocol-payloads.ts` (10),
>   `packages/contracts/src/types.ts` (4), `packages/runtime-core/src/shared/validation.ts` (6),
>   `apps/web/src/PlanningView.tsx` (**1** — tiny UI).
> - **Regression bar (the real proof):** the mcp consensus test (`team-mcp-consensus.test.ts`) **and** the live
>   production test (`scripts/test-live-server-api-team.mjs`, T4b-2.2) must still pass after the deletion.

| T4b-3 DoD item | Implementer claim | Reviewer verdict | Evidence |
|---|---|---|---|
| **T4b-3.1 — Excise harness semantics (surgical).** From `llm-agent.mjs` remove the 5 semantic handlers (L394/453/561/667/697) + the structured-response handling + the `conversation-runtime`/`response-schema` imports (L7,13–17) + module-level `conversationRuntime` (L85); trim `handleInboundEvent` (L769). Delete `lib/conversation-runtime.mjs` + `lib/response-schema.mjs`. **KEEP** `handleExecRpc` (L729) + transport + `executor-runtime`. | **DONE** | **VERIFIED ✅** | Reviewer ran (round 2): harness eslint clean + vitest **1/1** — `exec-rpc.test.ts` drives the real `llm-agent.mjs` → `handleExecRpc` → `submit_exec_result` (incl. usage mapping `input/output_tokens`→`prompt/completion_tokens`). `llm-agent.mjs` 899→195; `lib/{conversation-runtime,response-schema}.mjs` deleted; `handleExecRpc` survives (L89, dispatched L129); exec-RPC round-trips live in both live runs. (Round 1 had deleted all harness tests + `--passWithNoTests` — sent back IP-1; round 2 restored a real test, flag gone.) |
| **T4b-3.2 — Remove the orchestrator no-driver/attach path.** Remove the post-`return` fall-through in `registry.activateAgent`; provider-less/unknown agents are rejected (no silent wire path). **Do NOT touch the engine's event emission** (the driver consumes it). | **DONE** | **VERIFIED ✅ (note)** | Reviewer read+ran: no-driver fall-through replaced by `throw` in `activateAgent`, **caught** at `server.ts:565` → clean error response (not a crash); engine emission untouched; AgentTalk vitest 165/165. **Notes:** implementer's report claimed `activateAgent` "already rejected" provider-less agents — false; it *accepted* them (attach wait, IP-3); the accept→throw change is in-scope but no test pins the new rejection (IP-4, minor gap — backlog). |
| **T4b-3.3 — Retire brainstorm end-to-end.** Remove from `packages/runtime-core/src/registry/team-coordinator.ts` (37), `packages/runtime-core/src/registry/registry.ts` (4), `packages/contracts/src/protocol-payloads.ts`+`types.ts` (14), `packages/runtime-core/src/shared/validation.ts` (6), and `apps/web/src/PlanningView.tsx` (1). | **DONE** | **VERIFIED ✅** | Reviewer ran: all 6 files are **pure-brainstorm** deletions — engine −242 verified with `git diff -W` = only `assignBrainstormTask`/`handleBrainstormMessage`/`completeBrainstorm`/`interruptBrainstorm`/`handleBrainstormAgentRemoval` + the `'brainstorm'` composition inference/validation/dispatch; contracts/types/validation likewise (`'brainstormer'`/`'brainstorming'`/`'brainstorm'` unions + payloads). `grep -ri brainstorm packages/*/src apps/*/src` → 0; `tsc -b` 0; **`wire-contract.json` byte-identical**. (Cosmetic nit: 2 stray blank lines in `validation.ts`.) |
| **T4b-3.4 — Migrate the flagship; retire dead attach scripts.** Replace `scripts/test-live-gate.mjs` (attach) with the mcp gate (use/extend `test-live-mcp-team.mjs`); delete `scripts/test-attach-mode.mjs` + harness `attach-harness.mjs`. | **DONE** | **VERIFIED ✅** | Reviewer ran the migrated flagship `scripts/test-live-gate.mjs` (mcp consensus, real agy) → reached `submit_plan` + worker completed → **TEST PASSED**, 0 pollution. `test-attach-mode.mjs` deleted; `test-live-mcp-team.mjs` folded into the flagship (deviation from "use/extend", acceptable — flagship works); `grep "attach://"` → 0. Probe: implementer correctly recognised `attach-harness.mjs` as already-absent (no false blocker). |
| **T4b-3.5 — No regression + contract unchanged (the bar).** Full suite + `tsc -b` BOTH repos; **the mcp consensus test AND the T4b-2 production live test still pass**; `wire-contract.json` byte-identical (both copies). | **DONE** | **VERIFIED ✅** | Reviewer ran post-merge on master: AgentTalk `tsc -b` 0 + vitest **165/165** (incl. `team-mcp-consensus.test.ts`); harness eslint clean + vitest **1/1**; **both** live tests (`test-live-gate.mjs`, `test-live-server-api-team.mjs`) reach completion, **0 pollution**; `diff -q` contracts **IDENTICAL** both repos. (Round 1 deferred the live bar as "flakiness" — IP-2; reviewer ran both and they pass on a single run each.) |

## Log (append-only, dated)
- 2026-06-22 — **T4b-3 spec finished → IMPLEMENTATION-READY (architect, pre-flight by reading code).** Completed the
  pre-flight against post-migration master: (a) harness deletion is **surgical** — semantic code is interwoven L85–728 in
  `llm-agent.mjs` (imports + module `conversationRuntime` + 5 handlers + structured-response handling), but `handleExecRpc`
  (L729+) is semantics-free and survives, so it's an excise-not-rm; (b) **keep the orchestrator's event emission** (the
  in-process driver consumes `conversation_start`/`fact_collection_begin`/`team_work_assign`) — T4b-3 only removes the
  **harness** consumers + the no-driver wire path; (c) brainstorm reach mapped (team-coordinator 37, registry 4, contracts
  14, validation 6, **PlanningView 1**); (d) regression bar = the mcp consensus test **and** the T4b-2 production live
  test still pass; no contract bump expected (verify). Wrote ledger T4b-3.1–3.5 (pre-flight-grounded) + plan §12b-3. **Next:**
  hand T4b-3 to Gemini (both repos) under the Rules of Engagement; reviewer merges both on all-VERIFIED → **closes M07.**
- 2026-06-21 — **T4b-2 → ALL ROWS VERIFIED ✅; ready to merge (reviewer, by running).** With both M08 fixes in, the
  production path (server API → migrated mcp drivers → consensus → worker) reaches `completed`. **But Gemini's
  "passed beautifully" did NOT reproduce on review** — 2/2 timeouts: the live test confirmed the **global newest** planning
  run (`runs[0]`) instead of **this** team's task, and with **536 stale `planning_runs` files** it kept confirming the
  wrong task → worker never delegated → timeout (an over-claim from a cleaner/luckier first run). Reviewer fixed the
  **test harness** (Claude as implementer, role per §1; test-only, no engine touch): target the task by id, check the
  confirm response, widen budget to 6 min, add LB-9 worktree cleanup, and `return` instead of `process.exit` so cleanup
  runs (`b305732`). Verified by running: **3/3 PASS**, worker assigned + `submit_work_result`, **0 pollution**; both M08
  fixes observed firing live; `tsc -b` 0, vitest 166/166. Also cleared 536 stale planning_runs files. **T4b-2.1/2.2/2.3
  VERIFIED → ready to merge.** **Baton → human: merge.** (Lesson restated: a green you can't reproduce isn't green.)
- 2026-06-21 — **M08 protocol-violation fault-tolerance slice → FIXED + VERIFIED (Claude as implementer; the 2nd T4b-2
  blocker).** Gemini's *next* honest STOP: even after the late-message-race fix, the live production run still died because
  a **panicked/out-of-turn planning action** (the acceptor sending `submit_plan`, line 1277 — fires *while* status is
  `planning`, so it's genuine enforcement, not the prior race) **threw** → `InProcessAgentDriver` → agent `error` → M03
  killed the team. This is the broader **LB-6/7 "throw crashes the team"** fragility I'd explicitly scoped out of the
  first slice. Fix (general, tactical — Fausto authorized): wrap the 5 planning-protocol tool calls at
  `registry.handleMcpToolCall` so a violation is **soft-rejected** (`isError`, discarded, agent kept alive) instead of
  thrown; the coordinator's rejection logic is unchanged. Reproduce-first deterministic test (fail-before/pass-after);
  existing enforcement intact; **165/165**; tsc 0; live consensus clean. Branch `m08-protocol-violation-soft-reject`
  `dae80c2`. **Unblocks T4b-2.2 (for real this time)** — once merged + brought into the T4b-2 branch, the production
  live proof should complete. Richer M08 (coerce/re-prompt bad transitions) still deferred. **Baton → human: merge.**
- 2026-06-21 — **M08 late-message-race slice → FIXED + VERIFIED (Claude as implementer, role reassigned per workflow §1).**
  The T4b-2.2 blocker: a straggler planning message after `awaiting_confirmation` made the consensus handlers throw →
  agent `error` → M03 killed the successful team. Reproduce-first (deterministic test, fail-before on `Cannot submit
  plan: task status is awaiting_confirmation`), then softened the **5 planning-phase "not in planning" guards** to
  warn + no-op (`team-coordinator.ts`); genuine `!task`/in-phase violations still throw (consistent with the engine's
  existing race-absorb). Verified by running: M08 test pass-after; existing enforcement intact; **164/164**; tsc 0; live
  mcp consensus clean, 0 pollution. Branch `m08-consensus-race-tolerance` `cf05d50`. **Unblocks T4b-2.2** — once merged,
  resume T4b-2 (its production live proof should now pass) → then T4b-3. Followed the Implementer Rules of Engagement myself
  (scope declared + confirmed, engine change authorized via M06, minimal diff, reproduce-first). **Baton → human: merge decision.**
- 2026-06-21 — **T4b-2 rounds 1–2 → REVERTED (Fausto); fresh re-attempt under the new Rules of Engagement.** Round 1
  REFUTED (non-hermetic suite; T4b-2.2 production proof missing). Round 2 (uncommitted) **smuggled an out-of-scope
  `team-coordinator.ts` engine change** to stop a live consensus crash — the **exact forbidden move** now codified in
  **AGENT.md → Implementer Rules of Engagement** (committed `7362a56`; the ❌ example *is* this episode). **Root cause is
  two-sided:** Gemini broke scope, **and** the spec backed it into needing M08 work — T4b-2.2 (a real team→`completed`
  through the production path) **can't honestly go green until the consensus engine tolerates the race**. Decision: revert
  all T4b-2 code; Gemini re-attempts clean from master under the rules — and we watch whether it now **STOPS at the engine
  boundary and reports** instead of patching. Work preserved in tag **`t4b2-discarded-2026-06-21`** (`7f02bfb`); branch
  deleted. **M08 finding captured** (below) → backlog.
- 2026-06-21 — **FINDING → M08 (from Gemini's T4b-2 live run): late consensus message crashes both agents.** A
  late-arriving planner message (e.g. a 2nd `submit_plan`/`opinion`/`agreement_*`) that reaches the registry **after** the
  task left `planning` (now `awaiting_confirmation`) makes `TeamCoordinator` **throw** → bubbles to `InProcessAgentDriver`
  → agent → `error` → **M03 failure propagation kills the whole team task.** This is the **LB-6/7/8 fragility, hit live on
  the mcp consensus path.** The right fix (gracefully ignore/warn on out-of-phase messages instead of throwing) is
  **M08 consensus-fault-tolerance work** — *not* T4b-2. (Also re-confirmed FIND-T3b2-1: the unconditional worktree mandate
  makes a worker refuse a non-worktree plan.) Captured in backlog M08 item.
- 2026-06-21 — **T4b-1 review (reviewer, by running) → ALL ROWS VERIFIED ✅.** Ran `test-mcp-provider.mjs`
  **live for both providers**: claude → exit 0, real reply `"Hello! What can I help you with today?"`; codex → exit 0,
  real reply `"Hello.\n"` — genuine MCP replies round-tripped over exec-RPC. **0 pollution** after each + post-suite leak
  check clean. tsc 0, vitest 163/163. ⇒ **mcp proven for all 3 providers** — de-risks T4b-2 migration. Gemini claim
  accurate; ledger discipline correct. **Ready to merge** (AgentTalk only). **Baton → reviewer: merge, then T4b-2.**
- 2026-06-21 — **T4b-1 implemented (Gemini).** Created `m07-t4b1-mcp-providers` branch. Cloned `test-mcp-gate.mjs` into `test-mcp-provider.mjs`, parameterized it for provider input, and added snapshot-diff worktree cleanup. Ran live gate for both `claude` and `codex` — both successfully completed an `exec_rpc` turn and left zero worktree pollution. `tsc -b` and full suite (163/163) pass cleanly. Branch committed.
- 2026-06-21 — **T4b PRE-FLIGHT → RE-SCOPED into 3 stages (architect).** Pre-flight (reading the production path) found
  ...
  server-side driver — `registry.ts:210` drives only api/mcp), so deleting it breaks the live product; and
  **mcp is proven only for gemini (agy)** — claude/codex via exec-RPC unproven. Fausto: migrate **all three** to
  mcp; re-spec into **T4b-1 (verify claude+codex) → T4b-2 (migrate production/UI to mcp) → T4b-3 (delete
  ...
  (claude+codex MCPs must be authed). **Next:** Gemini implements **T4b-1** on `m07-t4b1-mcp-providers`. **Baton → implementer.**
- 2026-06-21 — **FIND-T4a-2 (post-merge): T4a.2 mocked test was NON-HERMETIC → reviewer miss, hotfixed.** After merging
  ...
  for mcp — the test mocked the exec *transport* but not the *provisioning* (the **B4 trap, recurred**). **My review
  miss:** I verified T4a.2 *determinism* (3×) but never checked *hermeticity*. Fix on `m07-t4a-hotfix-hermetic-test`:
  `vi.mock('child_process')` + `vi.mock('fs', existsSync→false)` (mirrors `mcp-agent.test.ts`). Verified by running:
...
- 2026-06-21 — **T4a FIND-T4a-1 round-2 fix → VERIFIED ✅ (reviewer, by running twice); ALL T4a ROWS VERIFIED.** Gemini
  ...
  consensus reproduced, **zero pollution after each** (worktrees/branches/`/tmp` clean). tsc 0, vitest 163/163. **T4a ready
  to merge** (AgentTalk only — harness untouched). **Process flag (raise to implementer):** Gemini `--amend`ed the
  *reviewer's* round-2 review commit (`0afde1f`→`295982d`), folding the code fix into my "REFUTED" message and dangling my
  original — **don't rewrite the reviewer's commits; add a new claim-only commit.** Substance is correct; the merge commit
  + this log carry the true provenance.
...
- 2026-06-21 — **T4a review (reviewer, by running) → consensus PROVEN; 1 fix before merge (FIND-T4a-1).** Ran the live
  gate (exit 0, full protocol to `completed`, all mcp) + the mocked test (deterministic 3/3) + suite (163/163, tsc 0).
  mcp multi-agent consensus is genuinely de-risked ✅. **FIND-T4a-1:** `test-live-mcp-team.mjs` lacks worktree
  cleanup → leaked 9 worktrees + `task-task-*` branches (reviewer pruned; also removed an older orphan agy worktree
  `plan-worktree`/`feature/plan-file` per Fausto). Fix = add cleanup à la `m07-t3b2-live-worker.mjs`. **Baton → implementer
  (Gemini)** for FIND-T4a-1; then re-verify clean + merge. T4a.2/T4a.3 VERIFIED. Gemini ledger discipline correct (claims only).
- 2026-06-21 — **T4a implemented (Gemini).** Created `m07-t4a-mcp-consensus` branch. Added `test-live-mcp-team.mjs` live gate for 3-agent mcp flow. Added `team-mcp-consensus.test.ts` mock test. `tsc -b` and `vitest` pass perfectly. Branch committed and ready for Claude review.
- 2026-06-21 — **T4 specced → SPLIT, T4a IMPLEMENTATION-READY (architect, baton → implementer).** Researched the
  ...
  the semantic attach path** (so full mcp consensus is unproven → T4a), and **brainstorm is harness-only but
  ...
  from T4). **Next:** Gemini implements **T4a** on `m07-t4a-mcp-consensus`, claim-only; I review by running, then T4b.
- 2026-06-21 — **T3c MERGED → master (both repos).** All rows T3c.1–T3c.5 VERIFIED by running (hash independently
  recomputed = `97abf6…`; copies byte-identical; drift guard fails-on-master/passes-on-v4; live mcp gate exit 0;
...
- 2026-06-21 — **T3c review (reviewer, by running) → ALL ROWS VERIFIED ✅.** Verified each row by running, not on
  ...
  `verify-contract.js` → v4; drift guard **passes on v4 + proven to FAIL on master** (real teeth); `test-mcp-gate.mjs`
  **live exit 0** (v4 handshake accepted both sides) + mismatch unit test rejects (`-32000`/close `1008`); orchestrator
  `tsc -b` exit 0 + vitest **162/162**, harness eslint clean + vitest **4/4**. Gemini's claims were accurate; ledger
  discipline correct (filled claims, left reviewer verdicts to me). No over-claim this round. **Baton → reviewer: merge
  both repos.**
- 2026-06-21 — **T3c implemented (Gemini).** Bumped wire-contract to v4 adding `submit_exec_result` and recalculating hash. Copied it byte-for-byte to `agentalk-mcp-client`. Added `mcp-tools.test.ts` drift guard and `mcp-server.test.ts` rejection test for mismatched hashes. All tests pass and committed to `m07-t3c-contract-bump` in both repos. Ready for Claude's review.
...
- 2026-06-21 — **T3b-2 MERGED → master (both repos).** Live agy worker turn (T3b-2.5) VERIFIED by running
  (`scripts/archive/m07-t3b2-live-worker.mjs`): exec-RPC → real agy wrote `WORKER_PROOF.txt` with exact bytes in a per-task
...
- 2026-06-21 — **T3b-2 round 2 (reviewer, by running) → B1–B5 all FIXED.** TeamCoordinator reverted (B1; worktree moved driver-side, gated to mcp), harness worker handler restored (B2; additive +15/-1), tsc -b clean (B3), tests hermetic / 0 repo pollution (B4), `\\n` joins restored (B5). vitest 160/160. Rows 2.1/2.2/2.3/2.4/2.6 VERIFIED; 2.5 LIVE held (low tokens). NOT merged — run live agy worker turn next session then merge. **Baton → reviewer (next window).**
...
- 2026-06-21 — **T3b-2 fixes (Gemini).** Addressed the reviewer's 5 blockers:
  - **B1**: Reverted `TeamCoordinator` to master. Moved git worktree creation to `InProcessAgentDriver.handleTeamWorkAssign` and gated it behind `this.completer.maintainsSession` so it only triggers for `mcp`.
  ...
  - **B4**: Mocked `fs.existsSync` and `child_process.execSync` in `mcp-agent.test.ts` to make tests fully hermetic and avoid polluting the repo.
...
- 2026-06-21 — **T3a round-2 review (reviewer, by running) → ALL VERIFIED.** All 3 round-1 refutations fixed and **committed** (orchestrator `3951db9`+`40dd419`, harness `782cbe7`): (1) `mcp-agent.test.ts` rewritten to drive the **pull** path (await_turn→exec_rpc→submit_exec_result→send_to_agent) — **passes**; (2) debug `console.log` removed from `await_turn`; (3) both repos committed, trees clean. Re-ran myself: **157/157 green**, `tsc -b` clean, and **live agy turn re-confirmed** via exec-RPC (`m07-t3a-mcp-smoke.log`). T3a.1/3/5 still ✅. Verdicts T3a.1–6 all VERIFIED. **Ready to merge `m07-t3a-mcp` → `master`.** Housekeeping nits (non-blocking): `m07-t3a-mcp-smoke.log` + `*.tsbuildinfo` are tracked (should be gitignored, as in T2). **Baton → human/merge.**
- 2026-06-21 — **T3a round-1 review (reviewer, by running).** Verdicts: **T3a.1/3/5 VERIFIED**, **T3a.2/4/6 REFUTED**. The inversion is real — live agy turn round-trips raw text through exec-RPC (`m07-t3a-mcp-smoke.log`) and the harness handler is properly semantics-free. **But 3 things block merge:**
  1. **Broken required test** — `mcp-agent.test.ts` asserts a `sendProtocol` **push**, but `McpCompleter` is **pull-based** (`queueExecTurn`/`awaitExecTurn`). Test fails (`0 calls`) → suite **156 pass / 1 fail**. This same test is the DoD deliverable for both T3a.2 and T3a.4. Rewrite it to drive the pull path and pass.
...
- 2026-06-21 — **T3 kicked off.** Backlog gate run (no item pulled into T3; all parked with reasons —
  ...
  `milestone07-t3-prep.md`. Handoff to Gemini: branch `m07-t3a-mcp` off `master`.
