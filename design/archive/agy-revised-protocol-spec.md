...
* **Replay/Debug Hooks**: Introduce an orchestrator MCP command (`npm run replay -- --run=<file>`) that reads a run log and replays the exact sequence of turns through the state machine validator to isolate engine bugs.
