...
1. **The consensus leak is one orphaned file + its wiring.** Consensus vocabulary
   ...
   **exactly one place**: `lib/stub-bridge.js` — a hand-coded mock-planner state machine — plus the
   ...
   `design/archive/phase6-multi-agent-consensus-plan.md:304`. ⇒ The stub is **dead code** → **delete**, not
...
- `design/archive/phase6-multi-agent-consensus-plan.md:304` — annotate/remove the stale `--provider stub`
  reference (stub no longer exists).
- Ledger `design/archive/spike-harness-division-implementation.md` — created at kickoff.
