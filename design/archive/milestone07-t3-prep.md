...
to a second "completer" that, instead of fetching, sends an **exec-RPC** to the MCP harness and awaits
`{text, usage}`.
```
        ┌─ Completer:  complete(prompt[, sessionId]) → {text, usage} ──┐
 brain ▶│  ApiCompleter (T1/T2, done)  │  McpCompleter (T3, new)   │
...
- **[FAUSTO] Coexistence, not cutover.** Add the exec-RPC path **behind a flag**, keep the M05/M06 MCP
...
1. **🔴 Effect-fence (R2) — the load-bearing unknown.** A MCP exec **mutates a git worktree → it is
...
3. **🔴 Session state (R3) — BIG OPEN NODE (Fausto flagged, 2026-06-20).** *Not* a checkbox.
   **Key reframe: "brain" ≠ "memory".** Centralized-brain means *semantics/decisions* live server-side
   ...
   - **Memory** → the MCP's **native session** (`agy --continue`; claude/codex `--continue`/`--resume`,
   ...
   the MCP's native session → **no resend, harness stays dumb, brain stays central**; (3) orchestrator-
   held session map.
   ...
   - **Residual tensions to resolve in its own spike:** (a) **determinism** — orchestrator can't see the
     MCP's exact context (compaction); OK because protocol enforces on the *reply's* `message_type`, not
...
Cheapest probe: **one MCP agent, one exec-RPC round-trip, no consensus, no reconnect** — prove the
...
- [FAUSTO] Session model (**BIG NODE — give it its own spike**, see §3.3): lean **sessionId / native
  MCP session** ("brain ≠ memory"); stateless-resend is only spike scaffolding, not the end state.
- [FAUSTO] Which MCP to pilot T3a on (claude / codex / agy)?
