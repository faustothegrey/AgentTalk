...
describe('Team CLI-exec Consensus (mocked)', () => {
  ...
  it('runs a full planner-planner-worker flow to completion via mocked cli-exec harness', async () => {
    const plannerA = await registry.createAgent('planner-a', { provider: 'cli-exec', providerName: 'gemini', model: 'planner-a' });
    const plannerB = await registry.createAgent('planner-b', { provider: 'cli-exec', providerName: 'gemini', model: 'planner-b' });
    const worker = await registry.createAgent('worker-1', { provider: 'cli-exec', providerName: 'gemini', model: 'worker-1' });
