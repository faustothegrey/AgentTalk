...
describe('CLI-exec no-resend and recovery fallback', () => {
  ...
  it('T3b-1.5: sends only the latest turn across multiple turns (no resend)', async () => {
    ...
    const submitResult = async (text: string) => {
      const p = new Promise(resolve => {
        const handler = (call: any) => {
          if (call.name === 'send_to_agent' && call.agentId === 'cli-agent-1') {
    ...
    const driver = (registry as any).apiDrivers.get('cli-agent-1') as InProcessAgentDriver;
