...
describe('Registry MCP-exec agent lifecycle', () => {
  ...
  it('creates a mcp agent, sends exec-RPC, and handles exec result', async () => {
    const agent = await registry.createAgent('mcp-agent-1', {
      provider: 'mcp',
      providerName: 'gemini',
    });

    expect(agent.provider).toBe('mcp');
    ...
    expect(sendToAgentArgs.payload).toContain('hello from mocked mcp');
    
    expect(agent.status).toBe('ready');
  });

  it('handles team_work_assign by sending exec-RPC with cwd and parsing result to submit_work_result', async () => {
    const agent = await registry.createAgent('mcp-worker-1', {
      provider: 'mcp',
      providerName: 'gemini',
    });
    ...
    // 1. Pull the exec_rpc
    const turnResult = await registry.handleMcpToolCall('mcp-worker-1', 'await_turn', {});
    const content = JSON.parse(turnResult.content[0].text);
    ...
    await registry.handleMcpToolCall('mcp-worker-1', 'submit_exec_result', {
      text: '```json\n' + JSON.stringify({
